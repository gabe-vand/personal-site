...
- Edit mode has no undo in the UI. It does not need one urgently: every save is its own commit, so `git log --oneline --grep="from the page"` finds it and `git revert` undoes it.
